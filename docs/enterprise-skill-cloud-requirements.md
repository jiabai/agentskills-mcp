...
| 功能 ID | 功能名称 | 描述 | 优先级 |
|---------|----------|------|--------|
| F-041 | 邮箱验证码登录 | 登录不使用密码，输入邮箱后发送验证码，验证成功后登录 | P0 |
| F-042 | 注册复用验证码 | 注册流程复用同一套验证码发送与校验 | P0 |
| F-043 | 邮箱绑定复用验证码 | 绑定邮箱时复用同一套验证码发送与校验 | P0 |
| F-044 | 验证码安全控制 | 支持有效期、重发间隔、错误次数限制与风控 | P0 |
